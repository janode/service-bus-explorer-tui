...
In [src/app.rs](../../../src/app.rs), add the variant:
...
In [src/event.rs](../../../src/event.rs), add a match arm in `handle_modal_input()`:
...
In [src/ui/modals.rs](../../../src/ui/modals.rs), add a rendering function and call it from the main modal render dispatch:
...
If the modal submission triggers an async operation, add the sentinel dispatch in `main.rs`. See the [add-operation skill](../add-operation/SKILL.md) for the full sentinel dispatch pattern.
